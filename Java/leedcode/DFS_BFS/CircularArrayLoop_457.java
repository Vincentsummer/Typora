...
public class CircularArrayLoop_457 {
	/**
	 * 转化成图，目的是求有没有除自环外的环。因为是数组，每个节点的出度是1，所以理论上每个节点只要经过一遍就
	 * 可以（可理解为从每个节点出发DFS遍历后，经过的节点都相当于被遍历过，不用再次进入）（也可以理解为从每个节
	 * 点出发，要么可以发现合法的环，要么直接排除掉，所以每个节点只走一遍）
	 ...
	 public boolean circularArrayLoop(int[] nums) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            while (nums[nextIndex] <= 1000 && nums[nextIndex] != round && (nums[nextIndex] > 0 ^ negative)) {
                int temp = nextIndex;
                nextIndex = (nextIndex + (nums[nextIndex] % n) + n ) % n;
                if (temp == nextIndex) {
                    length = 1;
                    break;
                }
